...
class Table {
    ...
    public void updateIP(String IP, byte metrica, String IPDestino) {    	
		if (!IP.equals(this.IPHost)) {
			...
			} else if (!this.metrica.get(indexOfIp).equals((byte)1)) { //Se nao e meu vizinho
				//Atualiza a metrica
	...
	public void updateIP(String IP, byte metrica, String IPDestino, String IPDestinoNovaTabela) {
		//Se não foi eu quem enviei o IP para ele
		if (!IPDestinoNovaTabela.equals(this.IPHost)) {
			//Atualiza a tabela de IPs
    ...
    public void updateTable(Table novaTabela) {
        houveSinalDeVida.set(
                IP.indexOf(novaTabela.getHost()),
                Boolean.TRUE
        );
        List<String> novosIP = novaTabela.getIP();
        List<Byte> novasMetricas = novaTabela.getMetricas();
		List<String> IPsDestinos = novaTabela.getIPsDestino();
        for (int i = 0; i < novosIP.size(); i++) {
            byte metricaIncrementada = novasMetricas.get(i);
            metricaIncrementada += 1;
            this.updateIP(novosIP.get(i), metricaIncrementada, novaTabela.getHost(), IPsDestinos.get(i));
        }
    }
	...
	private List<String> getIPsDestino() {
